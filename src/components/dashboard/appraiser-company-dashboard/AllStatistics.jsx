...
const AllStatistics = ({ properties, views, bids, favourites }) => {
  ...
  const getHoldAndCancelledPropertyData = (orderId) => {
    const filteredProperty = properties.filter((property) => property.orderId == orderId)?.[0] || {};
  ...
  const {
    ...
    bids.forEach((bid) => {
      if (bid.appraiserUserId == userData.userId) {
        const {onHold: propertyOnHold, onCancel: propertyOnCancel} = getHoldAndCancelledPropertyData(bid.orderId);
    ...
    let currentPlanInfo = userData?.plans?.$values
      ...
      console.log({currentPlanInfo, userData})
    if (currentPlanInfo) {
      PlanCount = currentPlanInfo?.planName || '';
      NoOfPropertiesCount = userData?.noOfProperties;
